...
@section('content')
    <div class="card">
        <div class="card-body">
            <div>
                <h3 class="card-title"><i class='bx bx-book'></i> Tambahkan Buku</h3>
            </div>
            <div class="row">
                <div class="col-md-6">
                    <form action="{{ route('buku.store') }}" method="POST" enctype="multipart/form-data"> @csrf
                        ...
                        <div class="input-group">
                            <input type="text" class="form-control" id="defaultFormControlInput" placeholder="Pengarang"
                                aria-describedby="defaultFormControlHelp" name="pengarang">
                        </div>
                        @error('pengarang')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <div class="input-group">
                            <input type="number" class="form-control" id="defaultFormControlInput" placeholder="Stok Buku"
                                aria-describedby="defaultFormControlHelp" name="stok">
                        </div>
                        @error('stok')
                            <div class="text-danger">{{ $message }}</div>
                        @enderror
                        ...
                        <input required type="file" accept="image/png, image/jpeg" class="form-control" id="gambarBukuInput" placeholder="Gambar Buku"
